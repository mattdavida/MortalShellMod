...
function SummonWeaponFromKeybind(weapon_id)
    ...
    if not WEAPON_MAP[weapon_id] then
        print('Invalid weapon_id: ' .. tostring(weapon_id) .. ' (valid: 0-5)')
        return
    end
...
function SuperBallistazooka()
    local anim_montage = StaticFindObject(
    '/Game/MainChar/anims/shooting/Char_anims/Balista_Reload_Montage.Balista_Reload_Montage')
    local equip_montage = StaticFindObject(
    '/Game/MainChar/anims/shooting/Char_anims/Ballista_Equip_Montage.Ballista_Equip_Montage')
    local idle_to_aim = StaticFindObject(
    '/Game/MainChar/anims/shooting/Char_anims/Ballista_Idle_To_Aim.Ballista_Idle_To_Aim')
    local idle_to_reload = StaticFindObject(
    '/Game/MainChar/anims/shooting/Char_anims/Ballista_Idle_To_Reload.Ballista_Idle_To_Reload')
    local equip_instant_reload = StaticFindObject(
    '/Game/MainChar/anims/shooting/Char_anims/Ballista_Equip_InstantReload_Montage.Ballista_Equip_InstantReload_Montage')
...
local WeaponMod = {
    EnableWeaponModKeybinds = EnableWeaponModKeybinds,
    MaxOutBolts = MaxOutBolts,
    SuperBallistazooka = SuperBallistazooka,
    SummonWeaponFromKeybind = SummonWeaponFromKeybind,
}
